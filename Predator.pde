public class Predator extends Animal{
  ...
  public void reproducepredator(){if ((int)floor((float)this.getAge())%Predator.BIRTH_CYCLE==0&&this.isAlive()&&this.getAge()>=1&&!this.hungry()){int k=smalldeadpredator();
   for(int j=0;j<2;j++){
if(k!=-1) {this.setAge(this.getAge()+1);
predator[k]=new Predator(0,random(10,100),random(10,100),random(2,4),
true,random(.4,.8),random(10,14), new Vector(max(min((this.p.x+random(-30,30)),width),0),max(min((this.p.y+random(-30,30)),height),0)));}}}}
