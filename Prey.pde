public class Prey extends Animal{
  public static final double DANGER_HUNGER=30,MAX_AGE=30,DANGER_DIST=70,BIRTH_CYCLE=2,ADULT_AGE=10;
  ...
  public void reproduceprey(){if ((int)floor((float)this.getAge())%Prey.BIRTH_CYCLE==0&&this.isAlive()&&!this.hungry()&&this.getAge()>=1){
    for(int j=0;j<2;j++){
int k=smalldeadprey();
if(k!=-1) {this.setAge(this.getAge()+0.5); reproducetime++;
prey[k]=new Prey(0,random(10,30),random(10,30),random(2,4),true,
random(.4,.8),random(8,12), new Vector(max(min((this.p.x+random(-30,30)),width),0),max(min((this.p.y+random(-30,30)),height),0)));}}}}
